...
public static class SienarApplicationBuilder
{
	...
	private static readonly PrioritizedDictionaryOfLists<Action> _middlewares = new();
	...
	{
		/// <summary>
		/// The startup DI container
		/// </summary>
		public IServiceCollection StartupServices  => _startupServices;
	...
	extension(IHost app)
	{
		...
		public IHost UseSienar()
		{
			foreach (var plugin in _plugins)
			{
				plugin.ConfigureApplication(app, _serviceScope.ServiceProvider);
			}

			foreach (var middleware in _middlewares.AggregatePrioritized())
			{
				middleware();
			}
		...
		/// <summary>
		/// Adds prioritized middleware to the middleware pipeline
		/// </summary>
		/// <param name="priority">The priority at which to add the middleware</param>
		/// <param name="middleware">The middleware to add</param>
		/// <returns>the host application</returns>
		public IHost UseMiddleware(
			int priority,
			Action middleware)
		{
			_middlewares.AddWithPriority(priority, middleware);
			return app;
		}
	}
}
